...
function bindGeneralChatButton() {
  const generalBtn = document.querySelector('[data-room="general"]');
  if (generalBtn) {
...
async function init() {
  bindGeneralChatButton();
  initEmojiPanel();

  const supabaseReady = isSupabaseConfigured() && window.supabase;
  isDemoMode = !supabaseReady;

  if (isDemoMode) {
    if (isSupabaseConfigured() && !window.supabase) {
      showLoadError("Библиотека Supabase не загрузилась. Обнови страницу или проверь интернет.");
    }
    showDemoBanner();
...
init().catch((err) => {
  console.error(err);
  showLoadError("Ошибка запуска: " + err.message);
});
